...
** cache
   ...
   帖子被标记为ticked（gnus-summary-tick-article-forward 命令为 =M t= 或 =!= ）和dormant（gnus-summary-mark-as-dormant 命令为 =M ?= 或 =?= ）后通过命令 =gnus-jog-cache= 来存放在cache中，下次再读到此帖时会从cache中读取，cache内的帖子永远不会过期。
