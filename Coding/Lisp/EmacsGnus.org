...
* 快捷键
  ...
  | r                           | 用命令 =d= 即可添加一个r标记，下次不显示此帖，可能因为不喜欢没有读，想当于delete删除这个帖子       |
...
*** 具体使用
    ...
    * 开启gnus时状态为unplugged而非默认的plugged:
      ...
      #+end_src
* 使用BBDB作通讯录
  [[http://blog.petitepomme.net/post/28547901478/installing-and-configuring-bbdb-3][参考]]
** 安装
  下载bbdb： =git clone git://git.savannah.nongnu.org/bbdb.git=
  安装：
  #+BEGIN_EXAMPLE
  autoconf
  configure
...
(add-hook 'gnus-summary-mode-hook
  ...
  {{{srcend}}}
** 使用
   进入gnus后在帖子上 =;= 即可将作者邮件/姓名保存到bbdb文件中并提示添加备注。可以在bbdb buffer中进行其它操作：
   * =e= 修改field
   * =i= 添加field，例如：phone/address
* 离线收发邮件
